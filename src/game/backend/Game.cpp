	...
	WRAPIMPL_CUSTOM_SETTERS
		...
		},
		{
			...
			NATIVE_METHOD_AUTO( this ) {
				N_EXPECT_ARGS( 3 );
				if ( !m_state->IsMaster() ) {
					GSE_ERROR( gse::EC.GAME_ERROR, "Only the game master can submit automated player events" );
				}
				N_GETVALUE( caller, 0, Int );
				if ( caller < 0 || static_cast< size_t >( caller ) >= m_state->m_slots->GetCount() ) {
					GSE_ERROR( gse::EC.GAME_ERROR, "Automated event caller is out of bounds" );
				}
				auto& caller_slot = m_state->m_slots->GetSlot( static_cast< size_t >( caller ) );
				if ( caller_slot.GetState() != slot::Slot::SS_PLAYER || !caller_slot.GetPlayer() ) {
					GSE_ERROR( gse::EC.GAME_ERROR, "Automated events require an active player caller" );
				}
				N_GETVALUE( name, 1, String );
				{
					std::lock_guard guard( m_event_handlers_mutex );
...
void Game::DeclareVictory( GSE_CALLABLE, const victory_type_t type, const size_t winner_slot ) {
	...
	if (
		...
		type != VT_ECONOMIC && type != VT_DIPLOMATIC && type != VT_RETIREMENT
	) {
...
const bool Game::ParseVictoryType( const std::string& value, victory_type_t& result ) {
	...
	if ( value == "retirement" || value == "score" ) {
		result = VT_RETIREMENT;
		return true;
	}
	result = VT_NONE;
	return value.empty();
