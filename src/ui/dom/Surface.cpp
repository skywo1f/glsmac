	...
	Property(
		...
		[ this ]( GSE_CALLABLE, gse::Value* const v ) {
			...
			if ( TryParseColor( GSE_CALL, str, color ) ) {
				m_background.texture = g_engine->GetTextureLoader()->GetColorTexture( color );
				is_tiled = false;
			}
			else {
				auto* const texture_loader = g_engine->GetTextureLoader();
				auto* const cached_texture = texture_loader->GetCachedFilteredTexture( str );
				if ( cached_texture ) {
					m_background.texture = cached_texture;
					is_tiled = str.find( ":stretch" ) == std::string::npos;
				}
				else {
					...
					auto* texture = texture_loader->TryLoadCustomTexture( params.front() );
					if ( !texture ) {
					...
					for ( auto it = params.begin() + 1; it != params.end(); it++ ) {
						...
						const auto f_expect_args_fromto = [ &ctx, &si, &ep, &f, &args, &it ]( const uint8_t args_min, const uint8_t args_max ) {
							if ( args.size() < args_min || args.size() > args_max ) {
								GSE_ERROR( gse::EC.UI_ERROR, "Texture filter '" + f + "' expects " + ( args_min == args_max ? std::to_string( args_min ) : std::to_string( args_min ) + " to " + std::to_string( args_max ) ) + " parameters, got " + std::to_string( args.size() ) + ": " + *it );
							}
						};
						...
						if ( f == "stretch" ) {
							...
							if (
								!util::String::ParseInt( args.at( 0 ), x1 ) || !util::String::ParseInt( args.at( 1 ), y1 ) || !util::String::ParseInt( args.at( 2 ), x2 ) || !util::String::ParseInt( args.at( 3 ), y2 ) ) {
					...
					if ( result.is_owned ) {
						...
						texture_loader->CacheFilteredTexture( str, m_background.texture );
