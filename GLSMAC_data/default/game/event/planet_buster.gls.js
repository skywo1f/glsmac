...
const rehome_surviving_units = (game, destroyed_bases, missile_id) => {
	let destroyed_ids = {};
	...
	for (unit of game.um.get_units()) {
		const home_key = base_key(unit.home_base_id);
		if (
			unit.id == missile_id || unit.home_base_id == 0 ||
		...
		for (candidate of game.bm.get_bases()) {
			const candidate_key = base_key(candidate.id);
			if (
				candidate.get_owner().id != unit.owner ||
			...
			if (
				...
				(distance == best_distance && candidate.id < destination.id)
			) {
