...
their self-contained installed CVRs. The original base-game component selection
...
highest Alpha Centauri Score deterministically as the final standings leader,
excludes native Planet, ends the game through a serialized `retirement` terminal
state, and presents the score tally without awarding victory status or bonuses.
Retirement cannot be declared early. The Windows RelWithDebInfo build, all 149 native and
